...
while True:
    ...
    if event == "Quitter" or event == gui.WIN_CLOSED: # Permet de gerer l'evenement du bouton "Quitter"
        ...
        Y=[]
        ordre = defordre
        if (nbpoint==2) & (ordre>= 2):
            gui.Popup("Il n'existe pas de courbe pour ces parametres")
        else:
            if values["-ORDRE-"]:
            ...
            for i in range(int(nbpoint)):
                for t in range(2):
                    if t== 0:
                        X.append(float(values[f"-INPUT{i}-{0}"])) #Remplis la liste des coordonnees X
                    elif t== 1: 
                        Y.append(float(values[f"-INPUT{i}-{1}"])) #Remplis la liste des coordonnees Y
            ax.cla()   #Clear le graphique         
            ax=rgl.CalculMatrice(int(nbpoint), ordre, X, Y)
            fig_agg.draw() #Affiche le graphique
